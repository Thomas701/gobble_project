...
// penser a liberer
int createTextureMapEmpty(SDL_Window * window,SDL_Renderer * renderer, SDL_Texture * textureBackground) // 0 si ok, -1 sinon
{
    SDL_Texture * textureMapEmpty = NULL;
    textureMapEmpty = loadImage("Frames/map.png", renderer);
    if(!textureMapEmpty)
    {
        fprintf(stderr, "Error for load textureMapEmpty in createTextureMapEmpty : %s \n", SDL_GetError());
        return -1;
    }
    /*if (0 != SDL_SetTextureBlendMode(textureMapEmpty, SDL_BLENDMODE_MOD))
    {
        fprintf(stderr, "transparence textureMapEmpty impossible : %s\n", SDL_GetError());
        return -1;
    }*/
    ...
    {
        fprintf(stderr,"REnderClear impossible in createTextureMapEmpty : %s \n",SDL_GetError());
    }
    /*if(0 != SDL_RenderCopy(renderer, textureBackground, NULL, NULL))
    {
        fprintf(stderr,"Error SDL_RenderCopy for textureBackground in createTextureMapEmpty : %s \n",SDL_GetError());
    }*/
    
    
    SDL_Rect rectangle ;

    // recupere taille texture
    SDL_QueryTexture(textureMapEmpty, NULL, NULL, &source.w, &source.h);

    
    destination.x = (window_dimensions.w - destination.w)/2;
    destination.y = (window_dimensions.h - destination.h)/2;
    
    if (0 != SDL_RenderCopy(renderer, textureMapEmpty, NULL, &destination))
    {
        fprintf(stderr, "Error RenderCopy in createTextureMapEmpty for textureMapEmpty : %s \n", SDL_GetError());
        return -1;
    }
...
int main(int argc, char ** argv) 
{
    ...
    else // lancement interface graphique
    {
        ...
        // chargement de la texture background (besoin pour menu & partie)
        ...
        createTextureMapEmpty(window,renderer, textureBackground);
